...
int main() {
	...
	openlog("dhcpd", 0, 0);
	printf("Lineo DHCP Server (v%s) started", VERSION);
	log_pid();
    ...
    { 
		...
		if(server_socket == -1) 
           {
			printf("couldn't create server socket -- au revoir\n");
			exit(0);
		}
        ...
        {
			printf("couldnt get option from packet (MSG_TYPE) -- ignoring\n");
			continue;
		}
		
		if((client_socket = clientSocket(LISTEN_PORT, SEND_PORT, interface_name)) == -1) 
        {
			printf("couldn't create client socket -- i'll try again\n");
			continue;
		}
		...
		switch(state[0]) 
        {
            ...
            if(server_id == NULL) 
            {
                printf("get option on 0x36 failed! NAKing");
            ...
            {
                printf("server_id = %02x%02x%02x%02x\n", server_id[0], server_id[1],
            ...
            break;
            default:
            printf("unsupported DHCP message (%02x) -- ignoring",state[0]);
            break;
        }
